...
namespace App\Authentication\Infrastructure\Role;
...
final readonly class RoleIdNormalizer implements NormalizerInterface, DenormalizerInterface
{
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): string
    {
        ...
        return $this->iriConverter->getIriFromResource(Role::class, context: [
            'uri_variables' => [
    ...
    {
        return $data instanceof RoleId;
    }

    /**
     * @param class-string<RoleId> $type
     * @param array{}              $context
     */
    public function denormalize($data, string $type, ?string $format = null, array $context = []): RoleId
    {
        if (!\is_string($data)
            || \strlen($data) <= 0
            || !class_exists($type)
            || !is_a($type, RoleId::class, true)
        ) {
