...
namespace App\Authentication\Infrastructure\Organization;
...
final readonly class OrganizationIdNormalizer implements NormalizerInterface, DenormalizerInterface
{
    ...
    {
        if (!$data instanceof OrganizationId) {
        ...
        return $this->iriConverter->getIriFromResource(Organization::class, context: [
            'uri_variables' => [
    ...
    {
        return $data instanceof OrganizationId;
    }

    /**
     * @param class-string<OrganizationId> $type
     * @param array{}                      $context
     */
    public function denormalize($data, string $type, ?string $format = null, array $context = []): OrganizationId
    {
        if (!\is_string($data)
            || \strlen($data) <= 0
            || !class_exists($type)
            || !is_a($type, OrganizationId::class, true)
        ) {
