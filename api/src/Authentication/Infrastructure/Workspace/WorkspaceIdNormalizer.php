...
final readonly class WorkspaceIdNormalizer implements NormalizerInterface, DenormalizerInterface
{
    ...
    {
        if (!$data instanceof WorkspaceId) {
        ...
        return $this->iriConverter->getIriFromResource(Workspace::class, context: [
            'uri_variables' => [
    ...
    {
        return $data instanceof WorkspaceId;
    }

    /**
     * @param class-string<WorkspaceId> $type
     * @param array{}                   $context
     */
    public function denormalize($data, string $type, ?string $format = null, array $context = []): WorkspaceId
    {
        if (!\is_string($data)
            || \strlen($data) <= 0
            || !class_exists($type)
            || !is_a($type, WorkspaceId::class, true)
        ) {
